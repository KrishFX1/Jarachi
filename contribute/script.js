...
async function a() {
  const docData = await firestore.collection('translator').doc('index').get();
  var data = docData.data()
  i = data.index
  wordBox.value = words[i];
}
a()
...
submit.addEventListener('click', async function (e) {
  ...
  var translatedWord = translation.value;
  translations[i] = `INSERT INTO language
  VALUES ("${word}", "${translatedWord}");`

  i++;

  conversions.innerHTML = `Total conversions : ${i}`
  wordBox.value = words[i];
  translation.value = ""
...
let done = document.getElementById("done")
  ...
  if (translations.length < 1) {
    ...
    firestore.collection('translator').doc('index').update({
      "index": i,
    });
    firestore.collection("translator").add({
